...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
   ...
   public void updateCameraAndRender(float partialTicks, long nanoTime, boolean renderWorldIn) {
      ...
      if (!this.mc.skipRenderWorld) {
         ...
         RenderSystem.matrixMode(5889);
         RenderSystem.loadIdentity();
         RenderSystem.ortho(0.0D, (double)mainwindow.getFramebufferWidth() / mainwindow.getGuiScaleFactor(), (double)mainwindow.getFramebufferHeight() / mainwindow.getGuiScaleFactor(), 0.0D, 1000.0D, net.minecraftforge.client.ForgeHooksClient.getGuiFarPlane());
         RenderSystem.matrixMode(5888);
         RenderSystem.loadIdentity();
